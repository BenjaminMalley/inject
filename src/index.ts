...
export default class ProgramLoader {
  ...
  getFullyQualifiedTypeNameFromSymbol(symbol: ts.Symbol): string {
    ...
    const symbolTypeName = this.checker.getFullyQualifiedName(symbolType.getSymbol())
    ...
    if (ts.isFunctionDeclaration(node) && node.name) {
      ...
      if (name.startsWith(this.providerFunctionNamePrefix)) {
        ...
        this.providers.add({
          ...
          parameterTypes: signature.parameters.map(param => this.getFullyQualifiedTypeNameFromSymbol(param)),
          returnType: this.getFullyQualifiedTypeNameFromType(signature.getReturnType()),
        })
      }
    }
